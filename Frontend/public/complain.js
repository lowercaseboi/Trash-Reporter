...
let lat, lng; // Define latitude and longitude globally
    ...
    if (navigator.mediaDevices && navigator.mediaDevices.getUserMedia) {
        ...
        navigator.mediaDevices.getUserMedia(constraints)
            .then(function (streamObj) {
                ...
                // Set canvas dimensions to match the video track's settings
                ...
                // Display the media container and canvas element
...
// Function to update the canvas with live video and date/time overlay
function updateCanvas() {
    if (!isCameraOpen) return;

    // Create a video frame using ImageCapture API
...
async function submitComplaint() {
    const description = document.getElementById("description").value.trim(); // Get description from input
    if (!description) {
    ...
    formData.append('image', blob, 'complaint.jpg'); // Append the captured image as a Blob with a filename
